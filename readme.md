...
Prime
  ...
  + Performance
  Space complexity is O(N) because the program caches N prime numbers once they are calculated.  Time complexity for completely calculating N prime numbers during the worst case (first time calculation, cache is empty) is O(m * sqrt(m)) where m is the Nth prime number.
